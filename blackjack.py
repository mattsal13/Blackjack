...
## Create the deck. 4 of each rank--suit doesn't matter.
deck = dict()
for i in range(2, 11):
    deck[f'{i}'] = 4
deck['J'] = 4
...
def player_draw(hand_num):
    ...
    while get_player_count(hand_num) > 21 and player_hands[hand_num]['A'] > 0:
...
def dealer_draw():
    ...
    while get_dealer_count() > 21 and dealer_hand['A'] > 0:
        dealer_hand['A'] -= 1
        dealer_hand['A2'] += 1

    return random_draw

## Implements a round of blackjack.
def round():
    ...
    player_ranks = [[]]
    # Remember that player_draw(hand_num) both updates the player_hand DICT but returns the LIST of draws. 
    # Draw 2 cards to hand 0.
    player_ranks[0].append(player_draw(0))
    player_ranks[0].append(player_draw(0))
    ...
    def split(pair_card):
        ...
        if inp == 'y':
            ...
            # Deal the new cards
            player_ranks[0].append(player_draw(0))
            player_ranks[1].append(player_draw(1))
    ...
    ## Prompts the user to hit or stay on hand_num. Recall the form player_draw(hand_num)
    def hit_stay(hand_num):
        inp = input(f"Hit, stay or double down on hand {hand_num}? (h/s/d) ")
        if inp == 'h':
            new_card = player_draw(hand_num)
            player_ranks[hand_num].append(new_card)
            print(player_ranks)
            if get_player_count(hand_num) > 21:
                return
            else:
                hit_stay(hand_num)
    ...
    def dealer_turn():
        while get_dealer_count() < 17:
            new_card = dealer_draw()
    ...
    while pointer < len(player_hands) and totally_busted is True:
        if get_player_count(pointer) < 22:
    ...
    print(f"Dealer's cards: {dealer_ranks} --> {get_dealer_count()}")
    # Player's cards. 
    for i in range(len(player_hands)):
        print(f"Your cards: {player_ranks[i]} --> {get_player_count(i)}")

        if get_player_count(i) > 21:
            ...
            if get_dealer_count() > 21:
                print(f"Dealer busted. You win hand {i}.")
            else:
                if get_player_count(i) > get_dealer_count():
                    print(f"You win hand {i}.")
                elif get_player_count(i) < get_dealer_count():
